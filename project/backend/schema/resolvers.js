...
const VIDEO_STATE_CHANGED = 'VIDEO_STATE_CHANGED'
...
export const resolvers = {
  ...
  Mutation: {
    ...
    updateVideoState: async (_, { roomId, isMuted, isVideoOff, isScreenSharing, isSpeaking }, { user }) => {
      if (!user) throw new Error('Not authenticated')
      ...
      pubsub.publish(`${VIDEO_STATE_CHANGED}_${roomId}`, {
        videoStateChanged: {
  ...
  Subscription: {
    ...
    videoStateChanged: {
      subscribe: (_, { roomId }) => pubsub.asyncIterator([`${VIDEO_STATE_CHANGED}_${roomId}`])
    },
  },
