...
export default function CallControls({
  ...
  return (
    <div className="flex items-center justify-center gap-2 md:gap-3 p-2 md:p-4 glass-card overflow-x-auto">
      {/* Microphone */}
      <motion.button
      ...
      {!isScreenSharing && (
        <motion.button
          ...
          className={`px-2 md:px-4 h-10 md:h-12 rounded-full flex items-center gap-1 md:gap-2 text-xs md:text-sm font-medium transition-all flex-shrink-0 ${
            shareSystemAudio
        ...
        >
          ...
          <span className="hidden md:inline">
            {shareSystemAudio ? 'Audio ON' : 'Audio OFF'}
          </span>
        </motion.button>
      )}
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={onEndCall}
        className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-red-500 hover:bg-red-600 text-white flex items-center justify-center transition-all flex-shrink-0"
        title="End call"
      >
        <svg className="w-4 h-4 md:w-5 md:h-5" fill="currentColor" viewBox="0 0 20 20">
